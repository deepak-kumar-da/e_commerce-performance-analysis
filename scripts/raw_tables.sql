...
if OBJECT_ID ('bronze.dim_geolocation') is not null
	drop table bronze.dim_geolocation;

create table bronze.dim_geolocation(
...
if OBJECT_ID ('bronze.fact_order_items') is not null
...
if OBJECT_ID ('bronze.fact_order_payments') is not null
	drop table bronze.fact_order_payments;

create table bronze.fact_order_payments(
order_id nvarchar(50),
...
if OBJECT_ID ('bronze.fact_order_reviews') is not null
	drop table bronze.fact_order_reviews;

create table bronze.fact_order_reviews(
...
if OBJECT_ID ('bronze.fact_orders') is not null
...
if OBJECT_ID ('bronze.dim_products') is not null
...
if OBJECT_ID ('bronze.dim_sellers') is not null
	drop table bronze.dim_sellers;
...
if OBJECT_ID ('bronze.dim_product_category_name') is not null
	drop table bronze.dim_product_category_name;

create table bronze.dim_product_category_name(
